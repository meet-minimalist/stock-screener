...
def render_rrg_data_uri(start_date: str, end_date: str, interval: str = "1d",
    ...
    benchmark_label = "Nifty" if get_market(market).key == "in" else "SPY"
    try:
        tails = compute_rrg_tails(start_date, end_date, interval=interval,
                                  rrg_window=rrg_window, tail_weeks=tail_weeks,
                                  cache_dir=cache_dir, market=market)
        if not tails:
            return None
        fig = build_rrg_chart(tails, benchmark_label=benchmark_label)
